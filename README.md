...
```sh
npx skills add pierre-lecocq/claude-skills --skill=REPLACE_SKILL_NAME_HERE -y
```
...
Copy the ones you want into a consuming project's own `CLAUDE.md` or `.claude/rules/` (or link to them from there).

- [`golang.md`](./rules/golang.md) — opinionated Go project conventions (project layout, imports, dependencies, config/logging/REST/database patterns, testing). Assumes a specific personal stack (go-pkg-base, BetterStack, cobra) — fork or edit before reusing it on a project that doesn't share that stack.
- [`sql.md`](./rules/sql.md) — table/column/index naming and formatting conventions, stack-agnostic.
- [`makefile.md`](./rules/makefile.md) — Makefile conventions.
